 ...
 * `ref`, `onMounted` ou `useState` em imports de verdade — é por isso que
 ...
 * runtime do Nuxt, é o menor shim que faz `useRealtime.ts` rodar sem alterar
 * uma linha dele.
 *
 * Se um teste futuro exercitar um composable que usa outro auto-import ainda
 * não coberto aqui, o sintoma é um `ReferenceError` claro no próprio teste —
 * a correção é ACRESCENTAR o global que faltou, não reescrever o composable
 * para importar explicitamente algo que o Nuxt já auto-importa.
 */
 ...
 * composables usam (`useOrigemClienteId`, EF-00): ler o valor existente ou
 * criar com o inicializador na primeira chamada.
 */
...
/** `useApiBasePublica` (`app/composables/useApi.ts`) — só o suficiente para o socket ter uma URL. */
(globalThis as Record<string, unknown>).useApiBasePublica = () => 'http://localhost:0';
