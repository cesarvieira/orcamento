...
- **Conteúdo base:** Com conteúdo — toda tarefa mesclada nesta história fechou `PASS`/`APROVADA`,
  conferido nas duas fontes duráveis (ver "Confiança" abaixo; não reproduzidas por inteiro aqui —
...
- **Confiança:** Alta. Todo código citado abaixo (`servico.ts`, `rotas.ts`, `esquemas.ts`,
  `schema.ts`, a migration, `metas.vue`, `useMetas.ts`) foi lido linha a linha por este agente
  `docs`, cada citação aberta na linha exata. **A retificação desta rodada** (a #89 foi reprovada
  na revisão da rodada anterior por citar uma fonte que não existia — ver "O que deu errado" em
  MANUAL-07): o veredito `PASS`/`APROVADA` por tarefa tem **duas** fontes duráveis, nesta ordem de
  preferência, as duas conferidas com `gh issue view <n> --comments` / `gh api
repos/cesarvieira/orcamento/issues/comments/<id>` **antes** de citar:
  1. **evidência primária, por tarefa** — o comentário que `carimbar-issue.sh` publicou em cada
     issue (máquina escrevendo, não modelo): #85
     ...
     — cada um traz a tabela de gate (`build`/`test(N>0)`/`front`/`typecheck`/`lint`/`deadcode`/
     `contrato`/`navegacao`, `fails=0`/`skips_bloqueantes=0`) e o `REVISAO_DE_DIFF=APROVADO` com
     commit e timestamp. **Não** traz contagem numérica de testes nem de rotas.
  2. **registro consolidado** — o comentário de linhagem na história #21,
     [issuecomment-5465501063](https://github.com/cesarvieira/orcamento/issues/21#issuecomment-5465501063),
     que o condutor publicou depois da rodada anterior desta tarefa ter sido reprovada por citar uma
     fonte que só existia no prompt, nunca em lugar durável — é dali que vêm os números "251 testes
     / 18 arquivos" (#86) e "10 rotas / 0 quebradas" (#87), que **não** encontrei em nenhum
     comentário por tarefa ao ler #86/#87 inteiros — só nesta tabela consolidada.
     O número de testes de **#88** (264/19) tem fonte primária dupla e mais forte que as duas acima:
     citado com saída real de `pnpm run teste` em dois comentários de revisão do próprio #88 —
     [issuecomment-5465377622](https://github.com/cesarvieira/orcamento/issues/88#issuecomment-5465377622)
     ("Test Files 19 passed (19)" / "Tests 264 passed (264)") e, após o retrabalho de lint,
     [issuecomment-5465404039](https://github.com/cesarvieira/orcamento/issues/88#issuecomment-5465404039)
     (mesma suíte re-rodada contra `ce0b318`: "19 arquivos / 264 testes... `metas-dod.teste.ts` = 13,
     `metas.teste.ts` = 27" — que bate exatamente com a contagem que esta própria tarefa fez nos
     arquivos, `grep -c "^\s*it("`, ver "DoD §5.1" abaixo). Aritmética de conferência cruzada: 251 (#86,
     18 arquivos) + 13 (os testes novos de `metas-dod.teste.ts`, confirmados) = 264 (#88, 19 arquivos)
     — os dois números do comentário consolidado fecham exatos contra o único delta que eu podia
     verificar sozinho, o que aumenta a confiança neles mesmo sem uma citação primária própria de #86.
...
| D3 · vínculo 1:1 cofrinho↔RESERVA                | Criada junto, saldo 0, nunca compartilhada                                  | Concluído                                                      | `schema.ts:698` (`uniqueIndex`); `servico.ts:103-141` (`criarMeta`, a conta nasce antes da meta). Testado: `metas.teste.ts:155-165` (criada junto, saldo 0, tipo RESERVA) e `:167-171` (duas metas, contas distintas — a asserção "nunca compartilhada" vive só neste segundo `it()`, não no bloco inteiro)                                                                                                                                                               |
...
| EF07-MC-004 | **Os quatro commits de merge desta história têm corpo vazio** (`79269b6`, `de255fa`, `fe13fe2`, `e75c26d` — conferido com `git show -s --format=%B`), diferente do padrão que EF-06/#79 registrou (carimbo no corpo do merge). ⚠️ Isto **não** significa evidência ausente — corrigido nesta rodada: a evidência existe, durável, nos comentários de cada issue (o carimbo de `carimbar-issue.sh` por tarefa, mais o comentário de linhagem consolidado na #21 — ver "Confiança" acima). A regressão é só de **onde** a linhagem foi registrada, não de a linhagem existir. | Nenhum em rastreabilidade da prova em si — a evidência está em GitHub, só não no `git log`. Nenhum em comportamento.                                                                                                                                                                                                                                                                             | Baixa — registrar para a próxima história voltar a citar o carimbo no corpo do merge, o formato mais barato de encontrar |
...
- [x] Toda tarefa de construção desta história (#85 a #88) citada `PASS`/`APROVADA` pelos
      comentários de `carimbar-issue.sh` em cada issue e pelo comentário de linhagem consolidado na
      #21 (URLs em "Confiança" acima; conferido: não recarimbado por esta tarefa — fora do escopo
      de `docs`; ver EF07-MC-004 sobre a ausência de carimbo no corpo dos merges)
